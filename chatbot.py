...
# Cosa abbiamo? domanda -> embedding -> similarity search -> risultati all'LLM -> risposta

    def formatta_documento(documenti):
        return "\n\n".join([documento.page_content for documento in documenti])
    
# Quanto segue dà un'identità al nostro agente, lo prepara. C'è da una parte il "system" cioè il chatbot; dall'altra "human" cioè l'utente
...
# Siamo alla semantic search, cioè io devo creare un qualche strumento che deve saper trovare quale degli embeddings è pertinente con la domanda fatta
...
# Se è stata messa in input una domanda utente la risposta del chatbot consisterà nell'invocare la catena che abbiamo fatto con la domanda utente
    if domanda_utente:
